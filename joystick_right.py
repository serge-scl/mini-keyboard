"""Read PWM X/Y from a right-hand joystick (GP10/GP11) and send mapped keyboard HID commands while the joystick is touched."""
...
# Logical command mapping for clock sectors
...
# Electrode and sens_touch code mapping (adjust indices/codes to match your hardware)
# These constants make which electrode corresponds to which joystick explicit and
# avoid "8 vs 9" style inconsistencies throughout the file.
JOYSTICK_RIGHT_ELE = 9            # mpr121 index for the right joystick touch electrode
JOYSTICK_LEFT_ELE = 8             # other joystick electrode index (used when checking both)
JOYSTICK_TOUCH_CODE = 18          # sens_touch() return code that indicates joystick touch/start
STOP_TOUCH_CODE = 19              # sens_touch() return code that indicates stop/exit
...
def joystick_touch_active():
    """Return True if right joystick (JOYSTICK_RIGHT_ELE) is currently touched using mpr121, if available.
    ...
    try:
        # mpr121[JOYSTICK_RIGHT_ELE] corresponds to the right joystick touch electrode
        return bool(st.mpr121[JOYSTICK_RIGHT_ELE].value)
    except Exception as e:
        print("Error reading mpr121 directly:", e)
        return False


print("Waiting for right joystick touch (sensor value 18) to activate...")

# Main loop: wait for sens_touch to indicate JOYSTICK_TOUCH_CODE or [..] to start; stop on STOP_TOUCH_CODE.
...
while True:
    ...
    if sens_touch is not None:
        ...
        # Stop condition per your requirement (STOP_TOUCH_CODE)
        if val == STOP_TOUCH_CODE:
            print(f"Received stop code {STOP_TOUCH_CODE} from sens_touch — exiting.")
            break

        # Start condition: JOYSTICK_TOUCH_CODE or list containing it
        if val == JOYSTICK_TOUCH_CODE or (isinstance(val, (list, tuple)) and JOYSTICK_TOUCH_CODE in val):
            print("Right joystick touch detected — entering joystick tracking.")
        else:
            # Not the right joystick — keep waiting
            continue
    else:
        # sens_touch not available: fall through and try to run if the physical mpr121 indicates touch
        if joystick_touch_active():
            print("mpr121 indicates right joystick active — entering joystick tracking.")
        else:
            time.sleep(0.1)
            continue
    ...
    try:
        while True:
            ...
            if st is not None:
                try:
                    # Stop when joystick is not touched (both JOYSTICK_LEFT_ELE and JOYSTICK_RIGHT_ELE false)
                    if not st.mpr121[JOYSTICK_LEFT_ELE].value and not st.mpr121[JOYSTICK_RIGHT_ELE].value:
...
# End of script
print("joystick_right.py finished.")
