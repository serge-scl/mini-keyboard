...
def joystick_touch_active():
    """Return True if left joystick (ELE8 -> 17) is currently touched using mpr121, if available.
    ...
    try:
        ...
        return bool(st.mpr121[8].value)
    except Exception as e:
        print("Error reading mpr121 directly:", e)
        return False


print("Waiting for left joystick touch (sensor value 17) to activate...")

# Main loop: wait for sens_touch to indicate 17 or [17,18] to start; stop on value 19.
...
while True:
    ...
    if sens_touch is not None:
        ...
        # Start condition: single 17 or list containing 17
        if val == 17 or (isinstance(val, (list, tuple)) and 17 in val):
